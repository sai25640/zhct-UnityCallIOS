...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class MediaPlayer : MonoBehaviour
	{
		...
		public bool OpenVideoFromFile(FileLocation location, string path, bool autoPlay = true)
		{
            ...
            XDebugger.Log("m_VideoPath =" + m_VideoPath);
            if (m_Control == null)
			{
				Initialise();
			}
		...
		{
			bool result = false;
            XDebugger.Log("m_Control ="+ m_Control);
			// Open the video file
			if ( m_Control != null )
			{
				CloseVideo();
                ...
                XDebugger.Log("fullPath =" + fullPath);
                if (!string.IsNullOrEmpty(m_VideoPath))
				{
					...
					if (checkForFileExist && !System.IO.File.Exists(fullPath))
					{
                        ...
                        XDebugger.Log("[AVProVideo] File not found: " + fullPath);
                    }
					else
					{
						Debug.Log("[AVProVideo] Opening " + fullPath);
                        XDebugger.Log("[AVProVideo] Opening " + fullPath);
                        if (!m_Control.OpenVideoFromFile(fullPath))
						{
							Debug.LogError("[AVProVideo] Failed to open " + fullPath, this);
                            XDebugger.Log("[AVProVideo] Failed to open " + fullPath);
                        }
						else
						{
							SetPlaybackOptions();
				...
				else
				{
                    ...
                    XDebugger.Log("[AVProVideo] No file path specified");
                }
			}
            return result;
		}
